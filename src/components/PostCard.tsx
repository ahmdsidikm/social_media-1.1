...
function CommentItem({
  ...
  const isCommentLiked = commentLikes.some((cl: CommentLike) => cl.user_id === currentUser.id);
    ...
    <div className={`${depth > 0 ? 'ml-8 mt-2' : ''}`}>
      <div className="flex items-start gap-2 group/comment">
        ...
        <button
          ...
          className={`h-7 w-7 rounded-full bg-gradient-to-br ${avatarGradient} flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0 overflow-hidden hover:ring-2 hover:ring-blue-200 transition shadow-sm ${depth > 0 ? 'h-6 w-6 text-[9px]' : ''}`}
        >
          {commenterAvatar ? (
            ...
            <div className="rounded-2xl bg-white px-3 py-2 shadow-sm border-2 border-blue-200">
              ...
              <div className="flex items-center justify-between mt-1.5">
                ...
                <div className="flex gap-1">
                  <button onClick={handleCancelEdit} className="px-2 py-0.5 text-[10px] font-medium text-gray-500 hover:bg-gray-100 rounded-lg transition">
                    Batal
                  </button>
                  <button onClick={handleSaveEdit} disabled={!editText.trim()} className="px-2 py-0.5 text-[10px] font-semibold text-white bg-blue-500 hover:bg-blue-600 rounded-lg transition disabled:opacity-40">
                    Simpan
                  </button>
                </div>
              </div>
            ...
            <>
              <div className="rounded-2xl bg-white px-3 py-2 shadow-sm border border-gray-100/50 relative inline-block max-w-full">
                <div className="flex items-center gap-1 mb-0.5 flex-wrap">
                  ...
                  {isCommentEdited && (
                    <span className="text-[8px] text-gray-300 italic" title={`Diedit ${timeAgo(comment.updated_at!)}`}>
                      diedit
                    </span>
                  )}
                </div>
                ...
                <p className="text-[12px] text-gray-600 leading-relaxed break-words">{comment.content}</p>

                {/* Like badge on comment bubble */}
                {commentLikeCount > 0 && (
                  <button
                    onClick={() => setShowLikersList(!showLikersList)}
                    className="absolute -bottom-2 right-2 flex items-center gap-0.5 bg-white rounded-full px-1.5 py-0.5 shadow border border-gray-100 hover:shadow-md transition"
                  >
                    <svg className="h-2.5 w-2.5 text-red-500" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                    </svg>
                    <span className="text-[9px] font-semibold text-gray-600">{commentLikeCount}</span>
                  </button>
                )}
                  ...
                  <div className="absolute top-1.5 right-1.5">
                    ...
                    {showMenu && (
                      <>
                        ...
                        <div className="absolute right-0 top-6 z-20 w-32 rounded-xl bg-white shadow-xl border border-gray-100 py-1 overflow-hidden">
                          {onEditComment && (
                            <button
              ...
              <div className="flex items-center gap-3 mt-1 px-1.5">
                ...
                {onLikeComment && (
                  <button
                    ...
                    className={`text-[10px] font-medium transition ${isCommentLiked ? 'text-red-500' : 'text-gray-400 hover:text-red-400'}`}
                  >
                    {isCommentLiked ? 'Disukai' : 'Suka'}
                  </button>
                )}
                ...
                {depth < maxDepth && (
                  <button
                    ...
                    className="text-[10px] font-medium text-gray-400 hover:text-blue-500 transition"
                  >
                    Balas
                  </button>
                )}
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full rounded-full border border-gray-200 bg-white px-3 py-1.5 pr-8 text-[12px] text-gray-800 placeholder-gray-400 focus:border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-50 transition"
                  autoFocus
                  onKeyDown={(e) => {
              ...
              <button
                ...
                className="flex h-6 w-6 items-center justify-center rounded-full bg-blue-500 text-white hover:bg-blue-600 transition disabled:opacity-30 flex-shrink-0"
              >
                <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="mt-1">
              {!showReplies ? (
                <button
                  ...
                  className="flex items-center gap-1.5 text-[10px] font-medium text-blue-500 hover:text-blue-600 transition ml-1 mt-1"
                >
...
function CommentLikersList({
  ...
  return (
    <div className="mt-1.5 ml-1 rounded-xl bg-white border border-gray-100 shadow-lg overflow-hidden max-w-[240px]">
      <div className="flex items-center justify-between px-3 py-2 border-b border-gray-50">
        <div className="flex items-center gap-1.5">
          <svg className="h-3 w-3 text-red-500" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
          </svg>
          <span className="text-[10px] font-semibold text-gray-600">{commentLikes.length} suka</span>
        </div>
      ...
      <div className="max-h-32 overflow-y-auto p-1.5" style={{ scrollbarWidth: 'thin' }}>
        {loading ? (
          ...
          users.map(user => (
            <button
              key={user.id}
              onClick={() => { if (onViewProfile) onViewProfile(user); onClose(); }}
            ...
            >
              ...
              <span className="text-[11px] font-medium text-gray-700 truncate flex-1">{user.display_name}</span>
              {user.id === currentUser.id && (
                <span className="text-[8px] bg-pink-100 text-pink-500 px-1 py-0.5 rounded-full font-medium flex-shrink-0">Kamu</span>
              )}
            </button>
          ))
        )}
...
// ── Main PostCard ──
export function PostCard({ post, currentUser, onLike, onComment, onDeletePost, onDeleteComment, onEditComment, onLikeComment, onViewProfile, onEditPost }: Props) {
      ...
      {likeCount > 0 && (
        <div className="px-4 py-2.5 border-t border-gray-50">
                ...
                <>
                  ...
                  <div className="ml-2.5 flex-1 min-w-0">
                    <p className="text-xs text-gray-500 truncate">
                      ...
                      {likerUsers.length > 2 && (
                        <span>{' '}dan <span className="font-semibold text-gray-700">{likerUsers.length - 2} lainnya</span></span>
                      )}
                  ...
                  {remainingLikersCount > 0 && (
                    ...
                    >
                      ...
                      <svg className={`h-3 w-3 transition-transform duration-200 ${showAllLikers ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
          ...
          {showAllLikers && remainingLikersCount > 0 && (
            <div className="mt-3">
              <div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(249,250,251,0.8)', backdropFilter: 'blur(10px)', border: '1px solid rgba(0,0,0,0.05)' }}>
                <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
                  ...
                  <button onClick={() => setShowAllLikers(false)} className="h-6 w-6 rounded-full flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-black/5 transition">
                  ...
                  <div className="space-y-0.5">
                    {likerUsers.map((user) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-gray-800 truncate leading-tight">{user.display_name}</p>
