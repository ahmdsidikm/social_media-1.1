...
const deleteFileFromStorage = async (fileUrl: string): Promise<boolean> => {
  ...
  const { error } = await supabase.storage.from(storageInfo.bucket).remove([storageInfo.path]);
  if (error) { console.error('Failed to delete:', error.message); return false; }
  return true;
};
...
const deleteFilesFromStorage = async (fileUrls: string[]): Promise<void> => {
  ...
  for (const url of fileUrls) {
    ...
    if (storageInfo) {
      if (!bucketFiles[storageInfo.bucket]) bucketFiles[storageInfo.bucket] = [];
...
const getGroupsLastRead = (userId: string): string => {
  return localStorage.getItem(`sosmedku_groups_last_read_${userId}`) || new Date(0).toISOString();
};

const setGroupsLastRead = (userId: string): void => {
  localStorage.setItem(`sosmedku_groups_last_read_${userId}`, new Date().toISOString());
};
...
export function App() {
  ...
  const loadUser = async (userId: string) => {
    ...
    else localStorage.removeItem('sosmedku_user_id');
    setLoading(false);
  };
  ...
  const fetchPosts = useCallback(async () => {
    const { data: postsData } = await supabase.from('posts').select('*, users(*)').order('created_at', { ascending: false });
    if (postsData) {
      const postsWithDetails = await Promise.all(
        postsData.map(async (post: Post) => {
          const { data: likesData } = await supabase.from('likes').select('*').eq('post_id', post.id);
          const { data: commentsData } = await supabase.from('comments').select('*, users(*)').eq('post_id', post.id).order('created_at', { ascending: true });
          return { ...post, likes: likesData || [], comments: commentsData || [] };
        })
      );
  ...
  const fetchUnreadMessages = useCallback(async () => {
    if (!currentUser) return;
    const { count } = await supabase.from('messages').select('*', { count: 'exact', head: true }).eq('receiver_id', currentUser.id).eq('is_read', false);
    setUnreadMessages(count || 0);
  }, [currentUser]);

  const fetchUnreadGroups = useCallback(async () => {
    if (!currentUser) return;
    if (isOnGroupsPageRef.current) { setUnreadGroups(0); return; }
    ...
    const { count } = await supabase.from('group_messages').select('*', { count: 'exact', head: true }).in('group_id', groupIds).neq('user_id', currentUser.id).gt('created_at', lastRead);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetchUnreadMessages();
      if (!isOnGroupsPageRef.current) fetchUnreadGroups();
    }, 10000);
  ...
  useEffect(() => {
    ...
    const messageChannel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `receiver_id=eq.${currentUser.id}` }, () => {
        if (currentPage !== 'messages') fetchUnreadMessages();
      })
  ...
  const handleLogout = () => {
    ...
    setViewingUser(null);
    isOnGroupsPageRef.current = false;
    scrollPositions.current = { home: 0, messages: 0, groups: 0, search: 0, profile: 0 };
  };
  ...
  const handleComment = async (postId: string, content: string) => {
    ...
    await supabase.from('comments').insert({ post_id: postId, user_id: currentUser.id, content: content.trim() });
    await fetchPosts();
  };
  ...
  const handleDeletePost = async (postId: string) => {
    ...
    if (post) {
      ...
      if (post.image_url) {
        if (Array.isArray(post.image_url)) imageUrlsToDelete.push(...post.image_url.filter(Boolean));
        else if (typeof post.image_url === 'string') imageUrlsToDelete.push(post.image_url);
      }
      if ((post as any).images && Array.isArray((post as any).images)) imageUrlsToDelete.push(...(post as any).images.filter(Boolean));
      if ((post as any).media_url) {
        if (Array.isArray((post as any).media_url)) imageUrlsToDelete.push(...(post as any).media_url.filter(Boolean));
        else if (typeof (post as any).media_url === 'string') imageUrlsToDelete.push((post as any).media_url);
      }
      if (imageUrlsToDelete.length > 0) await deleteFilesFromStorage(imageUrlsToDelete);
      const commentsWithImages = (post.comments || []).filter((c: any) => c.image_url || c.media_url);
      if (commentsWithImages.length > 0) {
        const commentImageUrls = commentsWithImages.map((c: any) => c.image_url || c.media_url).filter(Boolean);
        if (commentImageUrls.length > 0) await deleteFilesFromStorage(commentImageUrls);
      }
    }
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    if (comment) {
      ...
      if (commentImageUrl) await deleteFileFromStorage(commentImageUrl);
    }
